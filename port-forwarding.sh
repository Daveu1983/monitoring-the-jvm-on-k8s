...
kubectl port-forward -n monitoring $(kubectl get pods --selector=app.kubernetes.io/name=grafana -n  monitoring --output=jsonpath="{.items..metadata.name}") 3000 > /dev/null 2>&1 &
...
google-chrome --args "http://localhost:9090/" "http://localhost:3000/" "http://localhost:9093/"
